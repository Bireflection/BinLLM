...
sys.path.append('/home/shisenyou/.local/lib/python2.7/site-packages/')
import logging
import csv
import cgi
import os
...
print OUTPUT_DIR_Decompile_PATHS
try:
...
logfile_path = os.path.join(OUTPUT_DIR, "BinLLM.log")
...
class OutputGraph(object):
    ...
    def output_global(cls, func_color="#c9e4d8", val_color="#a7d3a9", source_color="#11069d", sink_color="#9b0748"):
        ...
        graph = pydot.Dot("BinLLMGlobalOutput", graph_type="digraph")
        graph.set_node_defaults(shape="plain")
        ...
        if SPLIT_GLOBAL_GRAPH_BY_FUNCS:
            for func in graphed_funcs:
                split_func_graph = pydot.Dot(
                    "BinLLMGlobalOutput_SplitFunc{}".format(
    ...
    def _format_href_to_decompiled_funcs(cls, node):
        ...
        if OUTPUT_RELATIVE_PATHS:
            file_uri_string = os.path.join("/home/shisenyou/Taint",OUTPUT_FILEPATH_DECOMPILED_C_AND_DISASSEMBLY_HTML)+"#{}".format(href_addr)
        else:
...
def main():
    ...
    with open("{}".format(full_filepath), "w+") as f:
        tmp_dict = dict()
        tmp_dict.update(funcs_dict)
        tmp_dict.update(funcs_content_dict)
